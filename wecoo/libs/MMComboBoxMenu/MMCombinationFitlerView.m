...
//            CGFloat left = ((i == 0)?ButtonHorizontalMargin:self.width - ButtonHorizontalMargin - 100);
            CGFloat left = ((i == 0)?0:kScreenWidth/2);
            UIColor *backcolor = ((i == 0)?[UIColor clearColor]:[UIColor colorWithHexString:titleSelectedColor]);
            UIColor *titleColor = ((i == 0)?FontUIColorBlack:[UIColor whiteColor]);
            UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
            button.frame = CGRectMake(left, 0, kScreenWidth/2, PopupViewTabBarHeight);
            button.tag = i;
...
- (void)dismissWithIdex:(NSString *)index{
    if (self.superview) {
        [self.shadowView removeFromSuperview];
        
    }
    if ([self.delegate respondsToSelector:@selector(popupViewWillDismiss: WithIndex:)]) {
        [self.delegate popupViewWillDismiss:self WithIndex:index];
    }
...
- (void)callBackDelegate {
    if ([self.delegate respondsToSelector:@selector(popupView:didSelectedItemsPackagingInArray:atIndex:)]) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.15 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self dismissWithIdex:[NSString stringWithFormat:@"%ld",self.tag]];
