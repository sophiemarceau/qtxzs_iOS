...
+ (instancetype)itemWithItemType:(MMPopupViewMarkType)type titleName:(NSString *)title titleforKeyCode:(NSString *)code {
    MMItem *item = [[[self class] alloc] init];
    item.markType = type;
    item.title = title;
    item.code = code;
...
- (void)findTheTypeOfPopUpView {
    ...
    if (self.alternativeArray.count || self.menuflag ==1) {
