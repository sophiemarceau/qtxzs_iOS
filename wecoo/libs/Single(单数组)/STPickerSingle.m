...
{
    self.selectedTitle = [self.arrayData[row] objectForKey:_keyName];
    selectIndex = row;
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view
{
    ...
    if (component == 0) {
        ...
        [label setText:[self.arrayData[row] objectForKey:_keyName]];
        label.frame = CGRectMake(0, 0, kScreenWidth, 50*AUTO_SIZE_SCALE_X);
        label.font = [UIFont systemFontOfSize:20*AUTO_SIZE_SCALE_X];
...
- (void)selectedOk
{
    [self.delegate pickerSingle:self selectedTitle:self.selectedTitle selectedIndex:selectIndex];
    [super selectedOk];
}
...
- (void)setArrayData:(NSMutableArray *)arrayData
{
    ...
    _selectedTitle = [arrayData.firstObject objectForKey:_keyName];
