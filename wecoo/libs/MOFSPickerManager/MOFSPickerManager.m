...
- (MOFSAddressPickerView *)twocolumnaddressPicker {
    if (!_twocolumnaddressPicker) {
        _twocolumnaddressPicker = [MOFSAddressPickerView new];
    }
    return _twocolumnaddressPicker;
}
...
- (void)show2CloumnAddressPickerWithTitle:(NSString *)title cancelTitle:(NSString *)cancelTitle commitTitle:(NSString *)commitTitle commitBlock:(void(^)(NSString *address, NSString *zipcode))commitBlock cancelBlock:(void(^)())cancelBlock {
    self.twocolumnaddressPicker.toolBar.titleBarTitle = title;
    self.twocolumnaddressPicker.toolBar.cancelBarTitle = cancelTitle;
    self.twocolumnaddressPicker.toolBar.commitBarTitle = commitTitle;
    self.twocolumnaddressPicker.tag = 2;
    [self.twocolumnaddressPicker show2ColumnAddressPickerCommitBlock:^(NSString *address, NSString *zipcode) {
        if (commitBlock) {
            commitBlock(address, zipcode);
        }
    } cancelBlock:^{
