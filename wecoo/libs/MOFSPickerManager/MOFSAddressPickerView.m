            ...
            if (weakSelf.dataArr.count > 0) {
                ...
                if (!cityModel) {
                    ...
                    if (!districtModel) {
                        ...
                        address = [NSString stringWithFormat:@"%@-%@-%@",addressModel.text,cityModel.text,districtModel.text];
                        zipcode = [NSString stringWithFormat:@"%@-%@-%@",addressModel.value,cityModel.value,districtModel.value];
                    }
                }
...
- (void)show2ColumnAddressPickerCommitBlock:(void(^)(NSString *address, NSString *zipcode))commitBlock cancelBlock:(void(^)())cancelBlock {
    [self showWithAnimation];
...
- (void)getData {
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"city_data" ofType:@"json"];
...
- (void)searchType:(SearchType)searchType key:(NSString *)key block:(void(^)(NSString *result))block {
    ...
    NSString *valueName = searchType == SearchTypeAddress ? @"text" : @"value";
    NSString *type  = searchType == SearchTypeAddress ? @"value" : @"text";
