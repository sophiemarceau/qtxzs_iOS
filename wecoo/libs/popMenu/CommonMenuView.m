...
- (void)setUpUI{
    [[NSNotificationCenter defaultCenter]removeObserver:self name:kPostMenuSorted object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(withdrawInfo:) name:kPostMenuSorted object:nil];

    self.backgroundColor = [UIColor whiteColor];
    
//    [UIColor colorWithRed:61/255.0 green:61/255.0 blue:61/255.0 alpha:1];
    arrowPointX = self.width * 0.5;
    tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, kTriangleHeight, self.width, self.height)];
    tableView.delegate = self;
    tableView.dataSource = self;
    tableView.bounces = NO;
    tableView.rowHeight = 43*AUTO_SIZE_SCALE_X;
...
- (MenuTableViewCell *)tableView:(UITableView *)temptableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MenuModel *model = self.menuDataArray[indexPath.row];
    MenuTableViewCell *cell = [temptableView dequeueReusableCellWithIdentifier:NSStringFromClass([MenuTableViewCell class]) forIndexPath:indexPath];
    cell.indexpathrow = (int)indexPath.row;
    cell.menuModel = model;
    cell.tag = indexPath.row;
    cell.backgroundColor = [UIColor clearColor];
    if (indexPath.row ==self.menuDataArray.count-1) {
        cell.lineView.hidden = YES;
    }else{
        cell.lineView.hidden = NO;
    }
    if(indexPath.row != row){
        cell.menuLabelIcon.image = [UIImage imageNamed:@"icon-sort-way-gray-1"];
...
-(void)withdrawInfo:(NSNotification *)notification{
    ...
    MenuModel *model= [userInfo objectForKey:@"modelentity"];
    row= [[userInfo objectForKey:@"indexpath"] intValue];
//    NSLog(@"row----->%d",row);
//    MenuModel *model = self.menuDataArray[indexPath.row];
    if (self.itemsClickBlock) {
        self.itemsClickBlock(model.itemName,row +1,[userInfo objectForKey:@"IsSelectFlag"]);
    }
...
- (void)updateFrameForMenu{
    ...
    menuView.contentTableView.height = 43*AUTO_SIZE_SCALE_X * menuView.maxValueForItemCount;
    menuView.height = 43*AUTO_SIZE_SCALE_X * menuView.maxValueForItemCount + kTriangleHeight * 2 - 0.5;
...
#pragma mark --- 类方法封装
+ (CommonMenuView *)createMenuWithFrame:(CGRect)frame target:(UIViewController *)target dataArray:(NSArray *)dataArray itemsClickBlock:(void(^)(NSString *str, NSInteger tag ,NSString *selectFlag))itemsClickBlock backViewTap:(void(^)())backViewTapBlock{
    
    CGFloat menuWidth = frame.size.width ? frame.size.width : 210/2*AUTO_SIZE_SCALE_X;
    
    CommonMenuView *menuView = [[CommonMenuView alloc] initWithFrame:CGRectMake(0, 0, menuWidth, 43*AUTO_SIZE_SCALE_X * dataArray.count)];
