...
#define kDefaultTintColor   RGB_Color(0, 190.0, 255.0)
...
@property(nonatomic, strong) RFSegmentItem *segmentItem;
@end
        ...
        if (items.count >= 2) {
            for (NSInteger i =0; i<items.count; i++) {
                RFSegmentItem *item = [[RFSegmentItem alloc] initWithFrame:CGRectMake(init_x, init_y, itemWidth, itemHeight)
                                                                     index:i
                                                                     title:items[i]
                                                                  norColor:RedUIColorC1//[UIColor whiteColor]
                                                                  selColor:[UIColor whiteColor]//kDefaultTintColor
...
- (void)moveToIndex:(NSInteger)index
{
    ...
    RFSegmentItem *item = self.itemsArray[index];
    item.isSelected = YES;
}
