...
@interface ReportViewController ()<UITableViewDataSource,SelectSuccessDelegate,UITableViewDelegate,UITextViewDelegate>{
...
//    self.SubDescLabel.frame = CGRectMake(15, 0, kScreenWidth-(15+25)*AUTO_SIZE_SCALE_X,85*AUTO_SIZE_SCALE_X);
    ...
    self.subview.frame = CGRectMake(0, self.subtitleLabel.frame.origin.y+self.subtitleLabel.frame.size.height, kScreenWidth,64*AUTO_SIZE_SCALE_X);
    ...
    self.HeaderView.frame = CGRectMake(0,
                                       0,
                                       kScreenWidth,
                                       self.clientView.frame.size.height+9.75*AUTO_SIZE_SCALE_X
                                       +self.reportView.frame.size.height+9.75*AUTO_SIZE_SCALE_X
                                       + self.remarkView.frame.size.height+10*AUTO_SIZE_SCALE_X
                                       +self.subtitleLabel.frame.size.height
                                       +self.subview.size.height
                                       +15*AUTO_SIZE_SCALE_X+btn.frame.size.height);
...
-(void)setTradecode:(NSString *)tradecode{
//    NSLog(@"tradcode----->%@",tradecode);
    [self initTrade:tradecode];
}
...
-(void)initTrade:(NSString *)tradeString{
        ...
        if(IsSucess(result)){
            ...
            if (array != nil) {
                ...
                for (NSDictionary *temp in tradeArrray) {
                    NSString *codestring = [NSString stringWithFormat:@"%d",[[temp objectForKey:@"code"] intValue]];
                    if ( [codestring isEqualToString:tradeString]) {
...
{
    [MobClick event:kFromDetailProjectReportSubmitEvent];
...
-(void)BudgeTaped:(UITapGestureRecognizer *)sender
{
    ...
    [[MOFSPickerManager shareManger] showPickerViewWithDataArray:budgetList tag:11 title:nil cancelTitle:@"取消" commitTitle:@"确定" commitBlock:^(NSString *string) {
...
-(void)planTaped:(UITapGestureRecognizer *)sender{
    ...
    [[MOFSPickerManager shareManger] showPickerViewWithDataArray:planList tag:12 title:nil cancelTitle:@"取消" commitTitle:@"确定" commitBlock:^(NSString *string) {
...
-(void)dButtonClick{
    [MobClick event:kSelectClientEvent];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MobClick beginLogPageView:kFromProjectDetailPage];//("PageOne"为页面名称，可自定义)
}
- (void)viewWillDisappear:(BOOL)animated
{
    [super viewWillDisappear:animated];
    [MobClick endLogPageView:kFromProjectDetailPage];
}
...
- (UILabel *)SubDescLabel {
    if (_SubDescLabel == nil) {
        self.SubDescLabel = [CommentMethod initLabelWithText:@"如果您不确认客户适合哪个项目，可以在这里直接报备。直接报备后，我们人工分配项目经理，签约后根据项目确定赏金" textAlignment:NSTextAlignmentLeft font:14*AUTO_SIZE_SCALE_X];
...
- (UILabel *)subtitleLabel {
    if (_subtitleLabel == nil) {
        self.subtitleLabel = [CommentMethod initLabelWithText:@"如果不匹配当前项目，我们会推荐到更合适的项目中" textAlignment:NSTextAlignmentLeft font:13*AUTO_SIZE_SCALE_X];
